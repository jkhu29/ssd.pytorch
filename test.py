    ...
    for i in range(num_images):
        ...
        with open(filename, mode='a') as f:
            ...
            for box in annotation:
                f.write(
                    'label: ' + VOC_CLASSES[box[-1]] + ' '  + ' || '.join(str(b) for b in box[0:-1])+'\n')
        if cuda:
            x = x.cuda()
...
if __name__ == '__main__':
    ...
    if os.path.exists(os.path.join(args.save_folder, "test.txt")):
        os.remove(os.path.join(args.save_folder, "test.txt"))
    test_voc(args)
